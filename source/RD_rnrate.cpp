...
int main(int argc, char** argv){
  ...
  int opt;
  int det_id=0;
  double t_radon220=3.824/log(2);
  double ene_Po218=6.00235;
  double ene_Po214=7.68682;
  double ene_Po212=8.785;
...
//##############################
  ...
  //lat.DrawLatex(labelposx,0.6*h_poraw->GetMaximum(),Form("negative veto: %.1lf V",veto_neg));
  ...
  lat.DrawLatex(live*.5,0.2*show_rate_max,Form("Po214: %.1f +- %.1f cpd.",po214_rate_fit,po214_rate_fit_err));
  lat.SetTextColor(col_Po218);
  lat.DrawLatex(live*.5,0.1*show_rate_max,Form("Po218: %.1f +- %.1f cpd.",po218_rate_fit,po218_rate_fit_err));
  lat.SetTextColor(col_Po212);
  lat.DrawLatex(live*.5,0.3*show_rate_max,Form("Po212: %.1f +- %.1f cpd.",po212_rate_fit,po212_rate_fit_err));
