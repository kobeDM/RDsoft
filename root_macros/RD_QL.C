void RD_QL(){
  ...
  filename[0]="/home/msgc/RD1/data/BG/data/20230207/sub_2.dat";
  filename[1]="/home/msgc/RD3/data/BG/data/20230207/sub_2.dat";
  ...
  for(int i=0;i<file_num;i++){
    ...
    histSps[i]->GetXaxis()->SetTitle("mV");
    ...
    histSps_nc[i]->GetXaxis()->SetTitle("mV");
    histWFs[i]->GetXaxis()->SetTitle("clock(10MHz)");
    histWFs[i]->GetYaxis()->SetTitle("mV");
  ...
  for(int i=0; i< file_num; i++){
    ...
    cerr<<"livetime: "<<live_time[i]<<" days"<<endl;
    //normalize
    histSps_MeV[i]->Sumw2(); 
   ...
   for(int i=0; i< file_num; i++){
     ...
     lts[i][1]=new TLatex(spmin+0.6*(spmax-spmin),(histSps[i]->GetMaximum())*0.7,livestr[i].c_str());
     lts[i][4]=new TLatex(spMmin+0.6*(spMmax-spMmin),(histSps_MeV[i]->GetMaximum())*0.8,livestr[i].c_str());
     lts[i][2]=new TLatex(spMmin+0.6*(spMmax-spMmin),(histSps_MeV[i]->GetMaximum())*0.6,Po214str[i].c_str());
     lts[i][3]=new TLatex(spMmin+0.6*(spMmax-spMmin),(histSps_MeV[i]->GetMaximum())*0.7,Po218str[i].c_str());
     lts[i][5]=new TLatex(spMmin+0.6*(spMmax-spMmin),(histSps_MeV[i]->GetMaximum())*0.5,calstr[i].c_str());
   }
